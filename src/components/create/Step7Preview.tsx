...
export function Step7Preview({ state, onPreview, onBack, onGoToStep, uploading, uploadError }: Step7PreviewProps) {
  ...
  const hasCustomAudio = (state.customAudio?.length ?? 0) > 0;
  ...
  return (
    <div className="flex flex-col items-center gap-8">
      <div className="text-center">
        <p className="tracking-[0.3em] text-white/45 mb-4" style={{ fontSize: "clamp(0.82rem, 1.6vw, 1rem)" }}>STEP 07</p>
      ...
      <div className="w-full bg-white/3 border border-white/8 rounded-xl p-5 space-y-3">
        ...
        <SummaryRow
          label="背景音乐"
          value={hasCustomAudio ? "已自定义" : "默认音乐"}
        />
      </div>
