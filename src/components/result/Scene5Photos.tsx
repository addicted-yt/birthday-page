...
export function Scene5Photos({ sectionRef, giftImages }: Scene5PhotosProps) {
  ...
  const go = useCallback((dir: number) => {
    setDirection(dir);
    setCurrent((value) => (value + dir + giftImages.length) % giftImages.length);
    setProgressKey((value) => value + 1);
  }, [giftImages.length]);

  const goTo = useCallback((index: number, currentIndex: number) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => go(1), AUTO_INTERVAL);
    ...
    >
      <motion.div
        style={{
          ...
          marginBottom: "clamp(1.4rem,4vh,2.4rem)",
        }}
      ...
      <div className="relative flex items-center justify-center w-full">
        ...
        <div className="relative flex items-center justify-center" style={{ width: "100%", height: "clamp(300px, 55vh, 520px)" }}>
      ...
      <motion.div
        style={{
          ...
          marginTop: "clamp(1.4rem,4vh,2.4rem)",
        }}
        initial={{ scaleY: 0, opacity: 0 }}
        whileInView={{ scaleY: 1, opacity: 1 }}
        viewport={{ once: true }}
